									...
									<v-card class="mt-4" elevation="2">
										<v-card-text class="pt-4">
											...
											<v-card class="my-4 pa-4" color="grey-lighten-4">
												...
												<div class="text-h6">Сума покупки = 2500 грн</div>
											</v-card>
											...
											<v-card class="my-4 pa-4">
												...
												></v-text-field>
												<v-btn color="primary" @click="checkDiscountAnswer" :disabled="discount.showSolution"> Перевірити </v-btn>
											</v-card>

											<v-expand-transition>
												<div v-if="discount.showSolution">
													<v-expansion-panels>
														<v-expansion-panel>
								...
								<v-expansion-panel-text>
									<v-card class="mt-4" elevation="2">
										<v-card-text class="pt-4">
											...
											<v-card class="my-4 pa-4" color="grey-lighten-4">
												<div class="text-subtitle-1 mb-2">Вхідні дані:</div>
												<div class="text-h6">Номер місяця = 7</div>
											</v-card>
											...
											<v-card class="my-4 pa-4">
												...
												></v-text-field>
												<v-btn color="primary" @click="checkSeasonAnswer" :disabled="season.showSolution"> Перевірити </v-btn>
											</v-card>

											<v-expand-transition>
												<div v-if="season.showSolution">
													<v-expansion-panels>
														<v-expansion-panel>
															<v-expansion-panel-title>
																<v-icon color="success" class="mr-2">mdi-check-circle</v-icon>
...
function checkDiscountAnswer() {
	...
	} else if (discount.value.attempts >= 3) {
		discount.value.error = "Спробуйте ще раз. Після 3 спроб ви зможете подивитися розв'язок";
		discount.value.showSolution = true;
	} else {
...
function checkSeasonAnswer() {
	...
	} else if (season.value.attempts >= 3) {
		season.value.error = "Спробуйте ще раз. Після 3 спроб ви зможете подивитися розв'язок";
		season.value.showSolution = true;
	} else {
